<?php
    ...
    $query = "SELECT d.no_order, d.detail_order, m.nama_merk, t.nama_tipe, d.no_seri, d.tgl_kalibrasi, p.name_owner, d.calibrator, d.tgl_masuk, d.tgl_sertifikat, p.address_1, p.address_2, d.progres, k.message
            FROM detail d
            ...
            INNER JOIN kalibrator k ON d.calibrator = k.calibrator
    ...
    function body_sertif() {
        ...
        else if($data['progres'] == 2){
            $hal = 2;
        }
        $hal = "Sertifikat ini terdiri dari        " . $hal . "      halaman";
    ...
    // <--------------------------------------------------------------------PAGE 2------------------------------------------------------------>
    if($data['progres'] == 6 || $data['progres'] == 5){
        $pdf->AddPage();
        // Header
        ...
        function footer_sertif_2() {
            ...
            // Atur posisi untuk teks agar berada di dalam kotak
            $pdf->SetXY($leftMargin + 2, $topMargin + 1.5); // Set posisi X dan Y dengan offset 2 untuk padding
            $pdf->SetFont('Arial', '', 8);
            $pdf->Cell(0, 3.5, 'Dilarang keras mengutip/ memperbanyak dan/ atau mempublikasikan sebagian isi Sertifikat ini tanpa izin UPT BYSTLAA - PT. KAI', 0, 1, 'C');
            $pdf->Cell(0, 3.7, 'Sertifikat ini sah bila telah dibubuhi cap UPT BYSTLAA - PT. KAI', 0, 1, 'C');
        }
    ...
    if($data['progres'] == 6){
        ...
        function tabel_tegangan_3(){
            ...
            while ($ukur = mysqli_fetch_assoc($pengukuran)) {
                ...
                // $pdf->SetFont('times','',10); // Atur font dan ukuran

                // Menampilkan data dengan simbol Ohm
                $pdf->Cell(22, 5, $ukur['standar'], 1, 0, 'C'); // Menampilkan dalam sel
                $pdf->Cell(25, 5, $ukur['rata_rata'], 1, 0, 'C'); 
                $pdf->Cell(20, 5, $ukur['koreksi_standar'], 1, 0, 'C'); 
                $pdf->Cell(30, 5, $ukur['rata_rata_koreksi'], 1, 0, 'C');
            }
    ...
    if($data['progres'] == 2){
        ...
        function body_sertif_3_2() {
            ...
            $pdf->Cell(0, 0, 'Laboratorium Kalibrasi Kelistrikan', 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 9, 'Halaman ke 2 dari 2 halaman', 0, 1);
            // END BARIS 1
    ...
    if ($data['progres']==6){
        ...
        function body_sertif_4() {
            ...
            $pdf->Cell(0, 0, 'Laboratorium Kalibrasi Kelistrikan', 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 9, 'Halaman ke 4 dari 4 halaman', 0, 1);
            // END BARIS 1
        ...
        function tabel_tegangan_4(){
            ...
            $pdf->SetX($leftMargin);
            $pdf->SetFont('Arial', '', 10);
            $pdf->Cell(0, 5, 'Keterangan : ', 0, 0);
            $leftMargin = 73;
            $pdf->SetX($leftMargin);
            $pdf->SetFont('Arial', 'B', 10);
            $pdf->Cell(0, 5, 'Normal sesuai spesifikasi alat', 0, 1);
            ...
            $pdf->SetX($leftMargin);
            $pdf->SetFont('Arial', '', 10);
            $pdf->Cell(0, 5, 'Catatan : ', 0, 0);
            $leftMargin = 35;
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 5, $data['message'], 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 5, 'yang tertelusur ke SI unit melalui Transmille Ltd.', 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 5, 'Prosedur kalibrasi mengacu ke Instruksi Kerja IK-01/KL-01, IK-01/KL-02 dan IK-01/KL-03', 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 5, 'Metoda acuan : EURAMET/cg-15/v.01: EA Guidelines on the Calibration of Digital Multimeter,2007', 0, 1);
            $pdf->SetX($leftMargin);
            $pdf->Cell(0, 5, 'Ketidakpastian kalibrasi dinyatakan pada tingkat kepercayaan 95% dengan k=2', 0, 1);
